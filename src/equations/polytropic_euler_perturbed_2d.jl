...
"""
struct PolytropicEulerEquationsPerturbed2D{RealT <: Real} <:
       AbstractPolytropicEulerEquations{2, 3}
    gamma::RealT               # ratio of specific heats
    kappa::RealT               # fluid scaling factor
    epsilon::RealT

    function PolytropicEulerEquationsPerturbed2D(gamma, kappa, epsilon)
        gamma_, kappa_, epsilon_ = promote(gamma, kappa, epsilon)
        new{typeof(gamma_)}(gamma_, kappa_, epsilon_)
    end
end

function varnames(::typeof(cons2cons), ::PolytropicEulerEquationsPerturbed2D)
    ("rho", "rho_v1", "rho_v2")
end
...
"""
function initial_condition_convergence_test(x, t, equations::PolytropicEulerEquationsPerturbed2D)
...
"""
function initial_condition_weak_blast_wave(x, t, equations::PolytropicEulerEquationsPerturbed2D)
...
@inline function flux(u, orientation::Integer, equations::PolytropicEulerEquationsPerturbed2D)
    @unpack epsilon = equations
    ...
    if orientation == 1
        f1 = rho_v1
        f2 = rho_v1 * v1 + p / epsilon^2
        f3 = rho_v1 * v2
    else
        f1 = rho_v2
        f2 = rho_v2 * v1
        f3 = rho_v2 * v2 + p / epsilon^2
    end
    return SVector(f1, f2, f3)
end
...
@inline function flux_winters_etal(u_ll, u_rr, normal_direction::AbstractVector,
    ...
    f2 = f1 * v1_avg + p_avg / epsilon^2 * normal_direction[1]
    f3 = f1 * v2_avg + p_avg / epsilon^2 * normal_direction[2]
...
@inline function max_abs_speeds(u, equations::PolytropicEulerEquationsPerturbed2D)
    @unpack epsilon = equations
    rho, v1, v2 = cons2prim(u, equations)
...
@inline function cons2prim(u, equations::PolytropicEulerEquationsPerturbed2D)
    rho, rho_v1, rho_v2 = u
...
@inline function cons2entropy(u, equations::PolytropicEulerEquationsPerturbed2D)
    @unpack epsilon = equations
    rho, rho_v1, rho_v2 = u
...
@inline function prim2cons(prim, equations::PolytropicEulerEquationsPerturbed2D)
    rho, v1, v2 = prim
    rho_v1 = rho * v1
    rho_v2 = rho * v2
...
@inline function density(u, equations::PolytropicEulerEquationsPerturbed2D)
    rho = u[1]
    return rho
end
# TODO - Should we simply put the epsilon^2 in pressure instead?

@inline function pressure(u, equations::PolytropicEulerEquationsPerturbed2D)
    rho, rho_v1, rho_v2 = u
